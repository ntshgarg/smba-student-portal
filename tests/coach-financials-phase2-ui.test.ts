...
describe("coach Financials Phase 2 workflows", () => {
  it("provides a dedicated, coach-authorized Rapid Desk route", () => {
    ...
    expect(workspace).toContain('if (scope === "all") params.set("scope", scope)')
    expect(workspace).toContain('workspace.selectedLedger ? finderHref : "/coach"')
    expect(workspace).toContain("Back to player list")
    ...
    expect(workspace).toContain("Only players with an outstanding registration or monthly fee can be selected.")
